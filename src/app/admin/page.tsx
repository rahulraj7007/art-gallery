// src/app/admin/page.tsx - Fixed version with Orders tab error resolved
...
export default function AdminDashboard() {
  ...
  // Edit Artwork Modal Component
  const EditArtworkModal = () => {
    if (!editingArtwork) return null;

    const [formData, setFormData] = useState({
      title: editingArtwork.title || '',
      artist: editingArtwork.artist || 'Aja Eriksson von Weissenberg',
      collection: editingArtwork.collection || '',
      description: editingArtwork.description || '',
      medium: editingArtwork.medium || 'Oil on Canvas',
      dimensions: editingArtwork.dimensions || '',
      category: editingArtwork.category || '',
      year: editingArtwork.year || new Date().getFullYear(),
      availabilityType: editingArtwork.availabilityType || 'enquire-only' as 'for-sale' | 'enquire-only' | 'exhibition' | 'commissioned' | 'sold',
      price: editingArtwork.price?.toString() || '',
      inStock: editingArtwork.inStock ?? true,
      availableAsPaperPrint: editingArtwork.availableAsPaperPrint ?? true,
      availableAsCanvasPrint: editingArtwork.availableAsCanvasPrint ?? true,
    });

    const [selectedTags, setSelectedTags] = useState<string[]>(editingArtwork.tags || []);
    const [customTag, setCustomTag] = useState('');
    const [imageUrls, setImageUrls] = useState<string[]>(editingArtwork.imageUrls || []);
    ...
    const moveImage = (fromIndex: number, toIndex: number, isNew: boolean = false) => {
      if (isNew) {
        ...
        [urls[fromIndex], urls[toIndex]] = [urls[toIndex], urls[fromIndex]];
        setImageUrls(urls);
      }
    };
    ...
    const handleSubmit = async (e: React.FormEvent) => {
      ...
      try {
        ...
        const allImageUrls = [...imageUrls, ...newUploadedUrls];
        ...
        // Remove price field if not for sale
        if (formData.availabilityType !== 'for-sale') {
          delete (updatedData as any).price;
          delete (updatedData as any).inStock;
        }

        await updateArtwork(editingArtwork.id, updatedData);
            ...
            <div>
              ...
              {newImagePreviews.length > 0 && (
                <div className="mb-4">
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 p-4 border border-blue-200 rounded-lg bg-blue-50">
    ...
    <div className="flex justify-between items-center">
      ...
      <div className="flex items-center space-x-4">
        <span className="text-sm text-gray-600">
          {newInquiries} new • {unreadInquiries} unread • {totalInquiries} total
        </span>
      </div>
    </div>
        ...
        {inquiries.map((inquiry) => (
          <div
            ...
            className={`bg-white rounded-lg shadow overflow-hidden ${
              !inquiry.read ? 'border-l-4 border-indigo-600' : ''
            }`}
          >
            <div className="p-6">
                ...
                <div>
                  <div className="flex items-center space-x-3">
                    ...
                    <span className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium ${
                      inquiry.inquiryType === 'commission' 
                        ? 'bg-purple-100 text-purple-800'
                        : inquiry.inquiryType === 'purchase'
...
{/* Orders Tab - FIXED VERSION */}
{activeTab === 'orders' && (
  <div className="space-y-6">
    <div className="flex justify-between items-center">
      <h2 className="text-2xl font-bold text-gray-900">Orders</h2>
      <div className="text-sm text-gray-600">
        Total: {totalOrders} orders • Revenue: {totalRevenue.toLocaleString()} SEK
      </div>
    </div>
              ...
              <div className="px-4 py-4 sm:px-6">
                <div className="flex items-center justify-between">
                  ...
                  <div className="ml-2 flex items-center">
                    <p className="text-sm font-semibold text-gray-900">
                      {/* Fixed: Added null check for order.total */}
                      {order.total ? `${order.total.toLocaleString()} SEK` : 'N/A'}
                    </p>
                ...
                <div className="mt-2">
                  <p className="text-sm text-gray-600">
                    {order.items ? `${order.items.length} item${order.items.length !== 1 ? 's' : ''}` : 'No items'}
                  </p>
                  {/* Optional: Show shipping address if available */}
                  {order.shippingAddress && (
                    <p className="text-xs text-gray-500 mt-1">
                      Ship to: {order.shippingAddress.city || 'N/A'}, {order.shippingAddress.country || 'N/A'}
                    </p>
                  )}
                </div>
              </div>
...
{activeTab === 'overview' && (
  <div className="space-y-6">
    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
      <div className="bg-white rounded-lg shadow p-6">
        <h3 className="text-lg font-semibold text-gray-900 mb-4">Artwork Breakdown</h3>
        <div className="space-y-3">
          <div className="flex justify-between items-center">
            <span className="text-sm text-gray-600">For Sale</span>
            <span className="font-semibold">{forSaleArtworks}</span>
          </div>
          <div className="flex justify-between items-center">
            <span className="text-sm text-gray-600">Enquire Only</span>
            <span className="font-semibold">{enquireOnlyArtworks}</span>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-lg shadow p-6">
        ...
        <div className="space-y-3">
          <div className="flex justify-between items-center">
            ...
            <span className="font-semibold text-indigo-600">{newInquiries}</span>
          </div>
          <div className="flex justify-between items-center">
            <span className="text-sm text-gray-600">Unread Messages</span>
            <span className="font-semibold text-yellow-600">{unreadInquiries}</span>
          </div>
        </div>
      </div>
    </div>
        ...
        {/* Edit Artwork Modal */}
        {showEditArtworkModal && editingArtwork && <EditArtworkModal />}
